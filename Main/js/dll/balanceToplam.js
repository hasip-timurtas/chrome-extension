...
async function GetTrades() {
    ...
    $.post(url, orderParams).done(data => {
        ...
        var sonuc ={
            ...
            btcBalance: Number($(".balancedata-" + balanceName).html()),
            anaToplam: Number(toplam.toFixed(8)) + Number($(".balancedata-" + balanceName).html())
        }
        _db.ref(`cryptopia-bot/${user.name}/balance`).set(sonuc)

        var html =`<div><span>Orders Toplam <span class="text-success">${sonuc.ordersToplam}</span> ${baseCoin}</span></div>
                   <div><span>${baseCoin} Balance <span class="text-success">${sonuc.btcBalance}</span> ${baseCoin}</span></div>
                   <div><span>Ana Toplam <span class="text-success">${sonuc.anaToplam}</span> ${baseCoin}</span></div>`
...
async function CryFbSaveBasla(){
    var user = GetUser()
    if(user){
		LoadOpenOrders(user.name)
    }
}

async function LoadOpenOrders(userName){
